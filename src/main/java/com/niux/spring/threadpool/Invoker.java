...
public class Invoker {
    ...
    public static ThreadPoolExecutor serviceExecutor = new ThreadPoolExecutor(8, 8, 2, TimeUnit.HOURS, blockingQueue, new ThreadFactory() {
        @Override
            ...
            serviceExecutor.submit(new Runnable() {
                ...
                public void run() {
                    while (true) {
                        ...
                        logger.info("invoke once {}", Thread.currentThread().getName() + Thread.currentThread().getId());
                        try {
                            Thread.sleep(2);
